...
@pytest.mark.parametrize("overlapping_metadata_role", ["release_cut", "ledger_tip"])
def test_recovery_ledger_v2_rejects_recovered_metadata_role_overlap(
    overlapping_metadata_role,
):
    recovered = "d" * 40
    release_cut = "e" * 40
    ledger_tip = "f" * 40
    overlapping_sha = release_cut if overlapping_metadata_role == "release_cut" else ledger_tip
    physical = {
        "previous_release_tag": "v0.1.0",
        "commit_shas": [recovered, release_cut, ledger_tip],
        "recovery_ledger": {
            ...
            "entries": [
                ...
                {
                    "recovered_commit_sha": overlapping_sha,
                    "source_pr": 111,
                    "primary_cr": 98,
                    "source_pr_merged": True,
                    "source_primary_crs": [98],
                    "source_merge_commit_sha": "b" * 40,
                    "observed_source_merge_commit_sha": "b" * 40,
                    "changed_path_hashes_match": True,
                },
            ],
        },
    ...
    failures = evaluate_recovery_ledger(
        physical,
        expected_version="0.1.1",
        declared_scope={96, 98},
    )

    assert "RECOVERY_LEDGER_COMMIT_ROLE_OVERLAP" in failures
    assert "RECOVERY_LEDGER_COMMIT_COVERAGE_MISMATCH" not in failures
