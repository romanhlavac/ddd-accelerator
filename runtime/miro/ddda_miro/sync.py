...
from .model import ManagedArtifact, load_artifacts, remote_semantic, semantic_hash, update_artifact_from_remote
...
def _write_conflict(project_root: Path, artifact_id: str, local: dict[str, Any] | None, remote: dict[str, Any] | None,
    ...
    save_yaml(path, {
        "schema_version": 1, "conflict_id": f"{stamp}-{artifact_id}", "artifact_id": artifact_id,
        "reason": reason, "base_local_hash": base_local_hash, "base_remote_hash": base_remote_hash,
        "local": local, "miro": remote, "resolution": "pending",
...
def sync_project(config: ProjectConfig, client: MiroClient, *, direction: str, dry_run: bool, include_layout: bool,
    ...
    conflicts: list[str] = []
    ids = sorted(set(local_by_id) | set(remote_by_id) | set(mapping["items"]))
    ...
    for artifact_id in ids:
        ...
        if artifact and not remote_item and map_entry.get("miro_item_id") and not recreate_missing:
            ...
            if dry_run:
                ...
                path = _write_conflict(config.root, artifact_id, artifact.semantic(), None, base_local_hash, base_remote_hash,
                                       "mapped Miro item is missing or inaccessible; use explicit recreate after review")
                conflicts.append(str(path.relative_to(config.root)))
            continue

        if local_changed and remote_changed and local_hash != remote_hash:
            operations.append({"action": "conflict", "artifact_id": artifact_id})
            if dry_run:
                conflicts.append(f"miro/conflicts/<timestamp>-{artifact_id}.yaml")
            else:
                path = _write_conflict(config.root, artifact_id, artifact.semantic() if artifact else None, remote_data,
                                       base_local_hash, base_remote_hash,
                                       "semantic fields changed in YAML and Miro since common base")
                conflicts.append(str(path.relative_to(config.root)))
            continue
        ...
        if direction in {"pull", "both"} and remote_data and (remote_changed or not artifact):
            if artifact:
                ...
                if not dry_run:
                    ...
                    local_hash, local_changed = artifact.semantic_hash(), False
            else:
        ...
        if direction in {"push", "both"} and artifact and (local_changed or not remote_item):
            ...
            payload = artifact.to_miro_payload(config.project_id, parent_item_id=parent_id,
            ...
            if not dry_run and remote_item:
                ...
                mapping["items"][artifact_id] = {
                    "miro_item_id": str(remote_item["id"]), "item_type": artifact.item_type,
                    "yaml_path": str(artifact.source_path.relative_to(config.root)).replace("\\", "/"),
                    "frame_id": artifact.frame_id, "managed": True, "updated_at": utc_now(),
                }

        if artifact and remote_data and not dry_run:
            state["items"][artifact_id] = {
                "local_hash": artifact.semantic_hash(), "remote_hash": semantic_hash(remote_data),
    ...
    if not dry_run:
        ...
        report = {"schema_version": 1, "project_id": config.project_id, "board_id": config.board_id,
                  "direction": direction, "completed_at": utc_now(), "operations": operations, "conflicts": conflicts}
    ...
    return {"project_id": config.project_id, "board_id": config.board_id, "direction": direction, "dry_run": dry_run,
            "operations": operations, "operation_count": len(operations), "conflicts": conflicts, "conflict_count": len(conflicts)}
