...
from .anchor_contract import (
    _get_item, _item_matches, _protected_snapshot, _target_payload, detect_board_state, load_manifest,
)
...
from .anchor_evidence import (
    ...
    _verify_registry_item, _verify_registry_markdown,
)
...
__all__ = [
    ...
    "_verify_no_anchor_overlap", "_verify_registry_item", "_verify_registry_markdown",
]
...
def apply_remediation(client: MiroClient, manifest: dict[str, Any], source_sha: str | None = None):
    ...
    registry_markdown = _verify_registry_markdown(manifest)
    registry_preflight = _verify_registry_item(client, manifest, target=False)
    ...
    try:
        ...
        registry_remote = _verify_registry_item(client, manifest, target=True)
        ...
        return {
            "status": "PASS",
            "remediation_id": str(manifest["remediation_id"]),
            "source_sha": source_sha,
            "board_id": board,
            "initial_board_state": state,
            "frames": frame_result,
            "items": item_result,
            "parked_obsolete_items": parked,
            "images": {
                "asset_count": 17,
                "first_run": first,
                "second_run": second,
            ...
            "artifact_registry": {
                **registry_markdown,
                "miro_projection": registry_remote,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        report = apply_remediation(MiroClient(os.environ["MIRO_ACCESS_TOKEN"]), manifest, args.source_sha) if args.apply else {
            ...
            "asset_count": 17,
            "artifact_registry_mode": manifest["artifact_registry"]["mode"],
        }
        code = 0
