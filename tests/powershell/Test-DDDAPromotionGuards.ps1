...
Assert-True -Condition ($promotion -match 'Mode\s*=\s*"EXACT_VALIDATION_EVIDENCE"') -Message "Controlled executor nemá explicitní exact-validation CI evidence mode."
Assert-True -Condition ($promotion -match 'if\s*\(\$ControlledReleaseSource\)\s*\{[\s\S]{0,900}?EXACT_VALIDATION_EVIDENCE') -Message "Controlled executor neváže CI evidence na revalidovaný exact report/package/gate contract."
Assert-True -Condition ($promotion -match 'else\s*\{[\s\S]{0,900}?Assert-DDDAGitHubChecksPassed') -Message "Standard merge-first promotion nevyžaduje direct GitHub check runs."
