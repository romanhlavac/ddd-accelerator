...
if ($LASTEXITCODE -ne 0) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Nepodařilo se inicializovat Git repozitář projektu."
    }
    Invoke-DDDAPlatformGit -Repository $projectPath -Arguments @("checkout", "-b", "main") | Out-Null
}
if (-not [string]::IsNullOrWhiteSpace($RemoteUrl)) {
    Invoke-DDDAPlatformGit -Repository $projectPath -Arguments @("remote", "add", "origin", $RemoteUrl) | Out-Null
}
if (-not $NoInitialCommit) {
    Invoke-DDDAPlatformGit -Repository $projectPath -Arguments @("add", ".") | Out-Null
    try {
        Invoke-DDDAPlatformGit -Repository $projectPath -Arguments @("commit", "-m", "chore: initialize DDDA project") | Out-Null
    }
    catch {
