...
def one(s,a,b):
    if s.count(a)!=1: raise RuntimeError(f'expected one match for {a[:40]!r}, got {s.count(a)}')
...
for blocked,blockers in {150:[149],151:[149],152:[149],153:[149,150,151,152],154:[150,153]}.items(): c['dependencies'].append({'blocked':blocked,'blocked_by':blockers})
c['dependencies'].sort(key=lambda x:int(x['blocked']))
f=next(x for x in c['fields'] if x['name']=='Work Package'); names=[x['name'] for x in f['options']]; assert 'WP-14' not in names; f['options'].insert(names.index('Other'),{'name':'WP-14','color':'GREEN','description':'Multi-Model Workbench & Git-backed Model Synchronization.'})
M={
